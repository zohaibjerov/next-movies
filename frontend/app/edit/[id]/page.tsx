...
export default function EditMovie() {
    ...
    const fetchMovie = async () => {
      try {
        ...
        const response = await axios.get(
          `${apiBaseUrl}/api/movies/${params.id}`,
          {
            headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let posterUrl = formData.currentPoster;
      ...
      if (formData.poster) {
        ...
        posterUrl = uploadResponse.data.image;
      }
      ...
      await axios.put(`${apiBaseUrl}/api/movies/${params.id}`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="min-h-screen flex items-start justify-center bg-[#022c40] px-8 pt-24">
      <div className="max-w-4xl w-full p-8 rounded-lg">
        <h1 className="text-3xl font-bold mb-8 text-white">Edit movie</h1>
        <form
